...
const Banner = () => {
  ...
  const [headlinePosition, setHeadlinePosition] = useState({
    x: 0,
    y: 0
  });
  ...
  return <main className="page-load relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-background p-6">
      ...
      <div className="fade-in absolute left-6 top-6 z-20">
        <Button variant="outline" size="icon" onClick={resetPositions} title="Reset layout">
      ...
      <div ref={containerRef} className="relative z-10 w-full max-w-7xl" style={{
      aspectRatio: '6/1'
    }}>
        <div className="flex h-full w-full items-center justify-between gap-8 px-4">
          {/* Center: Logo and Brand name - Draggable */}
          <div className={`fade-in absolute left-1/2 top-1/2 flex flex-col items-center justify-center select-none ${dragging === 'logo' ? 'cursor-grabbing opacity-80' : 'cursor-move'} ${dragging === null ? 'transition-transform duration-200' : ''}`} style={{
          transform: `translate(calc(-50% + ${logoPosition.x}px), calc(-50% + ${logoPosition.y}px + ${BASE_LOGO_OFFSET}px))`
            ...
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mb-3 text-accent pointer-events-none">
